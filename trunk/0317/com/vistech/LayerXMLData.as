			...
			for(var i:int=0;i<XmlDataArray.length;i++){
				...
				switch(stationType){
					...
					case 'HYDRO_RR':
						...
						var imageIcon:DisplayObjectMarker;
						switch((XmlDataArray[i].级别).toString()){
							case "1":
								imageIcon = new DisplayObjectMarker(imgClassRR,-12,-12,24,1,rotation);
								break;
							case "2":
								imageIcon = new DisplayObjectMarker(imgClassRR,-8,-8,17,1,rotation);
								break;
							case "3":
								imageIcon = new DisplayObjectMarker(imgClassRR,-5,-5,10,1,rotation);
								break;								
							case "4":
								imageIcon = new DisplayObjectMarker(imgClassRR,-2,-2,5,1,rotation);
								break;
							default:
								imageIcon = new DisplayObjectMarker(imgClassRR,-7,-7,19,1,rotation);
								break;	
						}						
						style.rules[0].symbolizers.push(new PointSymbolizer(imageIcon));		
						objData.id = XmlDataArray[i].站码;					
