package com.vistech
{
    ...
    public class LayerKML
    {
         ...
         * */
        public function LayerKML(map:Map, resolutions:String=null, layerName:String='ImmediateData' )
        {
            this.map=map;
            mainlayer=new FeatureLayer(layerName);  
        ...
        public function drawKML(event:ResultEvent):void
        {
            ...
            for (var iPlacemark:int=0; iPlacemark < kmlPlacemark.length; iPlacemark++)
            {
                //线条
                ...
                else if (kmlPlacemark[iPlacemark].Polygon)
                {
                    this.drawPolygon(kmlPlacemark[iPlacemark], kmlStyleMap, kmlStyle);
        ...
        {
            var arrLatlanKml:Array=(kmlPlacemark.LineString.coordinates as String).split(" ");
            ...
            */
            ...
            style.rules[0].symbolizers.push(new PolygonSymbolizer(null, new Stroke(0x0000FF, 4)));
            ...
            obj.type = kmlPlacemark.type;
            obj.id = kmlPlacemark.id;
        ...
        private function drawPoint(kmlPlacemark:Object, kmlStyleMap:ArrayCollection, kmlStyle:ArrayCollection, useMarkerManager:Boolean=true):void
        {
            ...
            objData.description=kmlPlacemark.description;
            objData.id=kmlPlacemark.id;
            objData.type=kmlPlacemark.type;
        ...
        private function drawPolygon(kmlPlacemark:Object, kmlStyleMap:ArrayCollection, kmlStyle:ArrayCollection, useMarkerManager:Boolean=true):void
        {
        	
            var arrLatlanKml:Array=(kmlPlacemark.Polygon.outerBoundaryIs.LinearRing.coordinates as String).split(" ");
            ...
            style=new Style();
            style.rules[0]=new Rule();
            style.rules[0].symbolizers.push(new PolygonSymbolizer(new SolidFill(0x0033FF,0.5), new Stroke(0x0000FF, 1)));
            arrayComponents=new Array();
            for (var i:int=0; i < arrLatlanMap.length; i++)
            {
                arrayComponents.push(new org.openscales.core.geometry.Point(arrLatlanMap[i][0], arrLatlanMap[i][1]));
			...
			var arrLinearRing:Array = new Array();
			arrLinearRing.push(new LinearRing(arrayComponents));
            var feature:PolygonFeature=new PolygonFeature(new Polygon(arrLinearRing), obj, style, true);
            this.mainlayer.addFeature(feature);
            CursorManager.removeBusyCursor();
        }
    }
}